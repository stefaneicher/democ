...
// Option ADT: A Abstract Data Type since CircularBufferStruct defined in c file and the type internal structure is
...
int CircularBuffer_VerifyIntegrity(CircularBuffer self) { return self->values[self->capacity] == BUFFER_GUARD; }

int CircularBuffer_IsEmpty(CircularBuffer self) { return self->count == 0; }

int CircularBuffer_IsFull(CircularBuffer self) { return self->count == self->capacity; }
...
int CircularBuffer_Get(CircularBuffer self) {
  ...
  self->count--;
  if (self->outdex >= self->capacity) self->outdex = 0;

  return value;
}

int CircularBuffer_Capacity(CircularBuffer self) { return self->capacity; }
...
void CircularBuffer_Print(CircularBuffer self) {
  ...
  for (i = 0; i < self->count; i++) {
    ...
    if (currentValue >= self->capacity) currentValue = 0;
  }
